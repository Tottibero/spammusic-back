...
export class ContentsService {
  ...
  async remove(id: string): Promise<void> {
    ...
    const contentWithSpotify = await this.contentRepo.findOne({ where: { id }, relations: ['spotify'] });
    const contentWithArticle = await this.contentRepo.findOne({ where: { id }, relations: ['article'] });
    ...
    // Usually Cascade Delete handles this if configured in Entity.
    // If we want manual sync:
    if (contentWithSpotify && contentWithSpotify.spotify) {
      await this.spotifyRepo.remove(contentWithSpotify.spotify);
    }
    if (contentWithArticle && contentWithArticle.article) {
      await this.articleRepo.remove(contentWithArticle.article);
    }
  }
